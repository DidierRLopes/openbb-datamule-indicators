...
# Workaround: Fetch options synchronously
# for immediate use in widget registration (not ideal)
def fetch_options_sync(url):
    ...
    except requests.exceptions.RequestException:  # More specific exception
        return []
...
ipo_index_options = fetch_options_sync(IPO_INDEX_CSV_URL)
    ...
    "params": [
        {
            ...
            "required": True,
            "show": True,
            "value": ipo_index_options[0]["value"] if ipo_index_options else "",
            "description": "Select the IPO component type to display the count for.",
            "options": ipo_index_options
        }
    ],
    "data": {
        "table": {
...
async def ipo_index_widget(
    ...
    """
    logic = create_format1_indicator_endpoint_logic(IPO_INDEX_CSV_URL)
    return await logic(component_type=ipo_type)

# Consumer Confidence Widget
CONSUMER_CONFIDENCE_CSV_URL = (
...
consumer_confidence_options = fetch_options_sync(CONSUMER_CONFIDENCE_CSV_URL)
    ...
    "params": [
        {
            ...
            "required": True,
            "show": True,
            "value": consumer_confidence_options[0]["value"] if consumer_confidence_options else "",
            "description": "Select the Consumer Confidence component type.",
            "options": consumer_confidence_options
        }
    ],
    "data": {
        "table": {
...
async def consumer_confidence_widget(
    ...
    """
    logic = create_format1_indicator_endpoint_logic(CONSUMER_CONFIDENCE_CSV_URL)
    return await logic(component_type=confidence_type)
